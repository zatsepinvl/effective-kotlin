...
fun main() {
    val solution = CourseScheduleSolution()
    ...
    val result = solution.canFinish(100, list.toTypedArray())
}

private class CourseScheduleSolution {
    ...
    fun canFinish(numCourses: Int, prerequisites: Array<IntArray>): Boolean {
        ...
        for (i in 0 until numCourses) {
            if (hasLoop(i, graph, mutableSetOf())) {
                return false
            }
        }
        return true
    ...
    private fun hasLoop(start: Int, graph: MutableMap<Int, MutableList<Int>>, visited: MutableSet<Int>): Boolean {
        if (cache[start] != null) {
            return cache[start]!!
        }
